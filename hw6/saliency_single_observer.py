...
class SingleObserverSaliencyModel(nn.Module):
    ...
    def __init__(self):
        ...
        """
        super(SingleObserverSaliencyModel, self).__init__()
        self.tased_net = TASED_v2()
...
class SingleObserverSaliencyEvaluator(object):
    ...
    def __init__(self, model_path='saliency_single_observer.pth'):
        ...
        self.model = torch.load(model_path).to(self.device).eval().float()

        # Не забываем повторить преобразования во время инференса
        self.im_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(), 
    ...
    def evaluate(self, video_frames_path, observer_data_path, output_saliency_path):
        ...
        for frame_name in tqdm.tqdm(sorted(os.listdir(video_frames_path))):
            ...
            frame = padding(cv2.cvtColor(cv2.imread(frame_path), cv2.COLOR_BGR2RGB),
                            height=360, width=640).astype('float32') / 255.
            frame = self.im_transform(frame).to(self.device)

            observer_map = normalize_map(padding(
                 cv2.imread(os.path.join(observer_data_path,  'gaussians', frame_name), 
                 cv2.IMREAD_GRAYSCALE), height=360, width=640))[None].astype('float32')
