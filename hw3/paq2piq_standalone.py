...
class InferenceModel:
    ...
    def predict_with_grads(self, image):
        #image = self.transform(image)
        #image = image.unsqueeze_(0)
        image = image.to(device)
        bs = len(image)
        self.model.input_block_rois(self.blk_size, [image.shape[-2], image.shape[-1]], device=device, batch_size=bs)
...
class MetricModel(torch.nn.Module):
    def __init__(self, dev, model_path=f'{curdir}/weights/RoIPoolModel.pth', batch_size=8):
        global device
        super().__init__()
        device = dev
        self.model = InferenceModel(RoIPoolModel(), model_path)
